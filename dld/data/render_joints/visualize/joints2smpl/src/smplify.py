...
from customloss import (camera_fitting_loss, 
                        ...
                        body_fitting_loss_smplx22, 
                        body_fitting_loss_smplx55, 
                        )
...
class SMPLify3D():
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, init_jaw_pose, init_leye_pose, init_reye_pose, init_left_hand, init_right_hand, j3d, conf_3d=1.0, seq_ind=0, origin_data=None):
       ...
       # -------------Step 1: Optimize camera translation and body orientation--------
        ...
        global_orient.requires_grad = True
        camera_translation.requires_grad = True
        if self.joints_category=="SMPLX55":
            handjoint_loss_weight = 1000.0
        ...
        if seq_ind == 0:
            ...
            if self.joints_category=="SMPLX55":
                body_opt_params = [body_pose, betas, global_orient, camera_translation, init_jaw_pose, init_leye_pose, init_reye_pose, init_left_hand, init_right_hand]
            else:
                body_opt_params = [body_pose, betas, global_orient, camera_translation]
        else:
            betas.requires_grad = False
            if self.joints_category=="SMPLX55":
                body_opt_params = [body_pose, global_orient, camera_translation, init_jaw_pose, init_leye_pose, init_reye_pose, init_left_hand, init_right_hand]
            else:
                body_opt_params = [body_pose, global_orient, camera_translation]

        j3d_relative = j3d.clone()
        if self.joints_category=="SMPLX55":
            j3d_relative[:,25:40] = j3d_relative[:,25:40] - j3d_relative[:,20:21]
            j3d_relative[:,40:] = j3d_relative[:,40:] - j3d_relative[:,21:22]
            ...
            for i in tqdm(range(self.num_iters)):
                def closure():
                    ...
                    if origin_data is not None:
                        ...
                        npy168 = origin_data.to(betas)
                        smpl_output = self.smplx(global_orient=npy168[:,3:6],
                                                transl=npy168[:,:3],
                                                body_pose = npy168[:,6:69],
                                                jaw_pose = npy168[:,69:72],
                                                leye_pose = npy168[:,72:75],
                                                reye_pose = npy168[:,75:78],
                                                left_hand_pose = npy168[:,78:78+15*3],
                                                right_hand_pose = npy168[:,-45:],
                                                expression= torch.zeros([npy168.shape[0], 10]).to(npy168),
                                                betas = betas)
                    else:
                        # pass
                        smpl_output = self.smplx(global_orient=global_orient,
                    ...
                    model_joints[:,25:40] = model_joints[:,25:40] - model_joints[:,20:21]
                    model_joints[:,40:] = model_joints[:,40:] - model_joints[:,21:22]
                    
                    if self.joints_category=="SMPLX55":
                        loss = body_fitting_loss_smplx55(body_pose, preserve_pose, model_joints, camera_translation,
                                                    j3d_relative,
                                                    joints3d_conf=conf_3d,
                                                    joint_loss_weight=600.0,
                                                    hand_joint_loss_weight=handjoint_loss_weight,
                        ...
                        loss = body_fitting_loss_smplx22(body_pose, preserve_pose, model_joints, camera_translation,
                                                    j3d_relative,
                                                    joints3d_conf=conf_3d,
                                                    joint_loss_weight=600.0,
                                                    use_collision=self.use_collision, 
        ...
        with torch.no_grad():
            ...
            if self.joints_category=="SMPLX55":
                final_loss = body_fitting_loss_smplx55(body_pose, preserve_pose, model_joints, camera_translation,
                                            j3d_relative,
                                            joints3d_conf=conf_3d,
                                            joint_loss_weight=600.0,
                                            hand_joint_loss_weight=handjoint_loss_weight,
                ...
                final_loss = body_fitting_loss_smplx22(body_pose, preserve_pose, model_joints, camera_translation,
                                            j3d_relative,
                                            joints3d_conf=conf_3d,
                                            joint_loss_weight=600.0,
                                            use_collision=self.use_collision, 
