...
def body_fitting_loss_3d(body_pose, preserve_pose,
 ...
 # #####--- body fitiing loss -----
def body_fitting_loss_smplx55(body_pose, preserve_pose,
                         model_joints, camera_translation,
                         j3d, 
                         joints3d_conf,
                         sigma=100, 
                         joint_loss_weight=500.0,
                         hand_joint_loss_weight=800.0,
                         pose_preserve_weight=0.0,
                         use_collision=False,
                         model_vertices=None, model_faces=None,
                         search_tree=None,  pen_distance=None,  filter_faces=None,
    ...
    model_joints_body = model_joints[:,:25,:]
    model_joints_hand = model_joints[:,25:,:]

    j3d_body = j3d[:,:25,:]
    j3d_hand = j3d[:,25:,:]

    joints3d_conf_body = joints3d_conf[:25]
    joints3d_conf_hand = joints3d_conf[25:]
    joint3d_error_body = gmof((model_joints_body + camera_translation) - j3d_body, sigma)
    # joint3d_error_body = F.l1_loss(model_joints_body + camera_translation, j3d_body, reduction='none')
    # joint3d_error_body = (model_joints_body + camera_translation - j3d_body) ** 2
    joint3d_loss_part_body = (joints3d_conf_body ** 2) * joint3d_error_body.sum(dim=-1)
    joint3d_loss_body = ((joint_loss_weight ** 2) * joint3d_loss_part_body).sum(dim=-1)
    print('joint3d_error_body', joint3d_error_body.mean())
    print('joint3d_loss_body', joint3d_loss_body.mean())

    # print('joint3d_loss_body', joint3d_loss_body.mean())
    joint3d_error_hand = gmof((model_joints_hand) - j3d_hand, sigma)
    # joint3d_error_hand = (model_joints_hand - j3d_hand) ** 2
    # joint3d_error_hand = F.l1_loss(model_joints_hand, j3d_hand, reduction='none')
    joint3d_loss_part_hand = (joints3d_conf_hand ** 2) * joint3d_error_hand.sum(dim=-1)
    joint3d_loss_hand = ((hand_joint_loss_weight ** 2) * joint3d_loss_part_hand).sum(dim=-1)

    print('joint3d_error_hand', joint3d_error_hand.mean())
    print('joint3d_loss_hand', joint3d_loss_hand.mean())
    print('next')
    ...
    total_loss = joint3d_loss_body + joint3d_loss_hand # + pose_prior_loss + angle_prior_loss + shape_prior_loss + collision_loss + pose_preserve_loss
 ...
 # #####--- body fitiing loss -----
def body_fitting_loss_smplx22(body_pose, preserve_pose,
                         model_joints, camera_translation,
                         j3d, 
                         joints3d_conf,
                         sigma=100, 
    ...
    model_joints_body = model_joints[:,:22,:]
    joints3d_conf_body = joints3d_conf[:22]
    joint3d_error_body = gmof((model_joints_body + camera_translation) - j3d, sigma)
    joint3d_loss_part_body = (joints3d_conf_body ** 2) * joint3d_error_body.sum(dim=-1)
    joint3d_loss_body = ((joint_loss_weight ** 2) * joint3d_loss_part_body).sum(dim=-1)
    print('joint3d_error_body', joint3d_error_body.mean())
    print('joint3d_loss_body', joint3d_loss_body.mean())
    ...
    # print('collision_loss', collision_loss)
    # print('pose_preserve_loss', pose_preserve_loss.shape)
    total_loss = joint3d_loss_body
    return total_loss.sum()



def debug_fitting_loss_smplx(body_pose, preserve_pose,
                         model_joints, camera_translation,
                         j3d, 
                         joints3d_conf,
                         sigma=100, 
